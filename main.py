...
#   query.message.edit(text=change_domain_text.format(DOMAIN),
#                      reply_markup=MDISK_ACTIV_BTN,
#                      disable_web_page_preview=True)
...
async def feature_cmd_func(a, msg):
  user = msg.chat.id
  Name = msg.chat.first_name
  await a.send_message(user,
                       feature_txt.format(name=Name),
                       disable_web_page_preview=True)
...
# @bot.on_message(
#   filters.user(sudo_users) & filters.private & filters.command('info'))
...
@bot.on_message(filters.private
                & filters.command(['del_channel', 'remove_channel']))
async def delete_link(_, msg):
  ...
  await bot.send_message(user,
                         removed_chanel.format(name=NAME),
                         disable_web_page_preview=True)
...
async def add_api(_, msg):
  ...
  if len(msg.command) == 1:
    return await API_MSG.edit_text(
...
async def delete_api(_, msg):
  ...
  if not API_KEY:
    await msg.reply_text(
      "**You don't have connected Your API.**\n\nPlease Add Your API using\n`/api < Your_API >`"
    )
    return
  delDATA(user, 'API')
...
@bot.on_message(~filters.me & filters.private & filters.incoming
                & filters.caption & filters.media)
async def media_msgs(a, m):
  ...
  try:
    ...
    MSG = await m.reply_text(
      "Message Caption was So Long so I can't send it with Media...")
    asyncio.sleep(3)
  await MSG.delete()


@bot.on_message(~filters.me & filters.private & filters.incoming
                & filters.regex(url_ptrn))
